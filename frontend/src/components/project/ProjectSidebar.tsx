...
const trustItems = [
  ...
  {
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-brand-primary" aria-hidden="true">
  ...
  return (
    <div className="flex items-center gap-px" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: fullStars }).map((_, i) => (
        <svg key={`full-${i}`} width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-accent" aria-hidden="true">
          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
        </svg>
      ))}
      {hasHalf && (
        <svg width="12" height="12" viewBox="0 0 24 24" className="text-accent" aria-hidden="true">
          <defs>
      ...
      {Array.from({ length: emptyStars }).map((_, i) => (
        <svg key={`empty-${i}`} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-gray-300" aria-hidden="true">
          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
        </svg>
      ))}
    </div>
    ...
    <aside className={cn('flex flex-col gap-md', className)}>
      ...
      <div className="rounded-md border border-gray-200 bg-white p-lg shadow-card">
        ...
        <div className="mt-lg flex flex-col gap-sm">
          <a
            ...
            className={cn(
              'inline-flex h-[40px] items-center justify-center gap-sm rounded-sm text-sm font-medium text-white no-underline transition-colors',
      ...
      <div className="rounded-md border border-gray-200 bg-white p-lg shadow-card">
        <div className="flex items-center gap-md">
          <div className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full bg-brand-primary-pale text-sm font-semibold text-brand-primary">
            {advisorInitials}
          </div>
